...
function createItems(setName, itemHeadText, itemImgLink) {
...
function load() {
    ...
    function getParam(param) {
        ...
        if (urlParams.get(param) !== null) {
            let setName = urlParams.get(param)
            setName = setName.replace('%27', `'`)
    ...
    // setArrRaw.forEach(x => {
    //     x = x.replace(`'`, '')
    //     let newElement = x.split(' ').join('_').toLowerCase()
    //     setArr.push(newElement)
    // })

    if (getParam('set') == null) {
        return blank('No Set Defined in Parameters')
    }
    if (setArrRaw.includes(getParam('set'))) {
        let setName = getParam('set')
        createElements(setName)

        let artifacts = infoJSON[setName].artifacts
        let artifactsArr = []
        for (let x in artifacts) {
            artifactsArr.push(x)
        }
        artifactsArr.forEach(x => {
            let itemName = x
            let imgLink = infoJSON[setName].artifacts[itemName].img
            createItems(setName, itemName, imgLink)
        })

        createFooter(setName)

    } else {
        return blank(`No Set Found Under Name " ${getParam('set')} "`)
    }
